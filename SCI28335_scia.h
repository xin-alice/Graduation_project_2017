...
/*SCI程序使用说明*/
/*通过设置一个标识变量来选择是否对scope程序进行调用 */
/*在main初始化程序处调用SCI_Init初始化串口*/
/*串口初始化后，调用SCOPEINITNOLOOP，初始化scope*/
/*当C33结束计算，将结果存入双口RAM触发2407中断程序中调用INC_CLOCK和SAVETOBUFFER，保存数据*/
/*设置串口中断所在的int5,必须将int5跳转至SERIAL*/ 
/*0x9000 must define DATA*/
/**/
/*==================================================================================================================*/



/*在savetobuffer中设置一个变量来判断C33是否接受了在线修改的变量的地址和数值，即让SCIStart ＝ 2 保持两个定时周期*/


/*疑问:2407中断的执行，是否没有采用Timer中断，采用的是什么中断*/


/*必须重新计算波特率，240和2407地时钟频率不一样，ft，还要重新计算S01_K地值，关系到Clock*/


/*这个好像无法捕捉到启动过程地波形，或许很难捕捉，东雷手里地东西肯定要借鉴地*/
...
/*确认PC机发送过来地数据 结构到底是什么样子地，怎么觉得每个中断只送过来4位阿*/
